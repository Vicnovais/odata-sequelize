 ...
 * Original grammar by: https://github.com/auth0/node-odata-parser
 *
...
//$skip
skip                        =   "$skip=" a:INT {return {'offset': ~~a }; }
...
orderbyList                 = i:(id:identifier ord:(WSP ("asc"/"desc"))? {
                                    var result = [];
                                    result[0] = id;
                                    result[1] = (ord[1] || 'asc').toUpperCase();
                                    return result;
                                })
...
//$select
select                      =   "$select=" list:selectList { return { "attributes":list }; }
